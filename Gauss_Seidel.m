...
A=M(1:max,1:max);                                                          % Matriz A após pivoteamento por colunas.
b=M(1:max,1+max);                                                          % Matriz b após pivoteamento por colunas.
b=I*b;                                                                     % Matriz b é reorganizada após permutações de colunas em A.
end
for a=1:max                                                                % Percorrendo todos elementos da diagonal principal.
    if A(a,a)==0                                                           % Para verificar se são diferente de zero.
...
while((norma_rel>toler)&&(iter<iter_max))                                  % Erro e o numero de iterações são as condições para interromper a simulação
    ...
    for i=1:max
        var=abs(x(i)-v(i));                                                % Uma variavél armazena o valor do maior modulo no numerador.
          ...
          if abs(x(i))> norma_dem                                          % Se o valor do modulo de x atual é maior que que norma_dem
             norma_dem=abs(x(i));                                          % O denominador é atualizado. 
          end 
    end 
