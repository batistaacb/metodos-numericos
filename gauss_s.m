...
aux = size(M) ;
max = aux(1);                                                              % numero de linhas da matriz A, obtida atravéz do comprimento do vetor B.
...
for a=1:max                                                                % Percorrendo todos elementos da diagonal principal.
    if A(a,a)==0                                                           % Para verificar se são diferente de zero.
    disp('A reorganiza��oo por linhas e colunas foi ineficaz, abortar simula��o');
    return
    end
end
for i=1:max
...
% =========================================================================
% Constru��o da Matriz para as Itera��es do M�ttodo de Gauss-Jacobi
% =========================================================================
for i=1:max
...
while((norma_rel>toler)&&(iter<iter_max))                                  % Erro e o numero de iterações são as condições para interromper a simulação
    ...
    for i=1:max
        var=abs(x(i)-v(i));                                                % Uma variav�l armazena o valor do maior modulo no numerador.
