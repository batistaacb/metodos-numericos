...
for I = 1:M
    ...
    K23 = f3(X,Y1,Y2,Y3);  % { avaliar f3(x , y) }
    %
    Y1 = Y1t + H * K21 / 2;
    Y2 = Y2t + H * K22 / 2;
    Y3 = Y3t + H * K23 / 2;
    ...
    K33 = f2(X,Y1,Y2,Y3);  % { avaliar f2(x , y) }
    ...
    K43 = f2(X,Y1,Y2,Y3);  % { avaliar f3(x , y) }
    %
    Xt = A + I * H;
    Y1t = Y1t + H * (K11 + 2 * (K31 + K41) + K41) / 6;
    Y2t = Y2t + H * (K12 + 2 * (K32 + K42) + K42) / 6;
    Y3t = Y3t + H * (K13 + 2 * (K32 + K42) + K43) / 6;
    %
    fprintf('%2i \t %10.5f %10.5f %10.5f %10.5f \n',I,Xt,Y1t,Y2t,Y3t);
